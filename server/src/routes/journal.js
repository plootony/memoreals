...
router.get('/', (req, res) => {
  ...
  try {
    let rows = db.prepare('SELECT * FROM journal WHERE user_id = ? ORDER BY date DESC').all(req.user.userId)
...
router.post('/', (req, res) => {
  ...
  try {
    ...
    db.prepare('INSERT INTO journal (id, user_id, title, content, date, photos, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)')
      .run(entry.id, req.user.userId, entry.title, encryptField(entry.content, codeword), entry.date, JSON.stringify(entry.photos), now, now)
...
router.put('/:id', (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE journal SET title=?, content=?, date=?, photos=?, updated_at=? WHERE id=?')
      .run(updated.title, encryptField(updated.content, codeword), updated.date, JSON.stringify(updated.photos), now, row.id)
